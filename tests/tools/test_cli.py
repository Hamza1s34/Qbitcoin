...
from qbitcoin.cli import qrl as qrl_cli
...
class TestCLI(TestCase):
    ...
    @mock.patch('qrl.cli.qbit_pb2_grpc.PublicAPIStub', autospec=True)
    def test_state(self, mock_stub):
        ...
        m_node_state_resp = qbit_pb2.GetNodeStateResp()
        m_node_state_resp.ParseFromString(m_node_state_resp_serialized_to_string)

        mock_stub_instance = mock.MagicMock(name='this should be qbit_pb2_grpc.PublicAPIStub(channel)')
        mock_stub_instance.GetNodeState.return_value = m_node_state_resp

        mock_stub.name = 'this should be qbit_pb2_grpc.PublicAPIStub'
    ...
    def test_tx_transfer(self, mock_stub):
        ...
        mock_stub.name = 'a fake qbit_pb2_grpc.PublicAPIStub'
        mock_stub.return_value = mock_stub_instance
    ...
    def test_tx_token(self, mock_stub):
        ...
        attrs = {"name": "this should be qbit_pb2_grpc.PublicAPIStub(channel)",
                 "PushTransaction.return_value": m_push_transaction_resp}
        mock_stub_instance = mock.MagicMock(**attrs)
