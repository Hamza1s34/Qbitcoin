...
class TestPublicAPI(TestCase):
    ...
    def test_getNodeState(self):
        ...
        node_state = service.GetNodeState(request=qbit_pb2.GetNodeStateReq, context=None)

        # self.assertEqual(__version__, node_state.info.version)  # FIXME
        self.assertEqual(qbit_pb2.NodeInfo.UNSYNCED, node_state.info.state)
    ...
    def test_getKnownPeers(self):
        ...
        service = PublicAPIService(qrlnode)
        response = service.GetKnownPeers(request=qbit_pb2.GetKnownPeersReq, context=None)
    ...
    def test_getStats(self):
        ...
        self.assertEqual(qbit_pb2.NodeInfo.UNSYNCED, stats.node_info.state)
        ...
        self.assertEqual(20000000000000000, stats.block_last_reward)
        self.assertEqual(0, stats.block_time_mean)
        self.assertEqual(0, stats.block_time_sd)

        self.assertEqual(30000000, stats.coins_total_supply)
    ...
    def test_getStats_timeseries(self):
        with MockedBlockchain.create(10) as mock_blockchain:
            ...
            request = qbit_pb2.GetStatsReq(include_timeseries=1)
            stats = service.GetStats(request=request, context=None)
    ...
    def test_getObject(self):
        ...
        with set_qrl_dir('no_data'):
            ...
            Block.put_block_number_mapping(db_state,
                                           block.block_number,
                                           qbit_pb2.BlockNumberMapping(headerhash=block.headerhash),
                                           None)
            context = Mock(spec=ServicerContext)
            request = qbit_pb2.GetObjectReq()
    ...
    def test_getLatestData(self):
        with set_qrl_dir('no_data'):
            ...
            request = qbit_pb2.GetLatestDataReq(filter=qbit_pb2.GetLatestDataReq.ALL,
                                               offset=0,
                                               quantity=3)
    ...
    def test_getAddressFromPK(self):
        ...
        request = qbit_pb2.GetAddressFromPKReq(pk=bytes(pk))
    ...
    def test_getMiniTransactionsByAddress(self, mock_dev_config):
        ...
        with set_qrl_dir('no_data'):
            ...
            TransactionMetadata.update_tx_metadata(db_state, block, None)
            request = qbit_pb2.GetMiniTransactionsByAddressReq(address=alice_xmss.address,
                                                              item_per_page=10,
                                                              page_number=1)
    ...
    def test_getBalance(self):
        with set_qrl_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = qbit_pb2.GetBalanceReq()
            request.address = get_alice_xmss().address
    ...
    def test_getTotalBalance(self):
        with set_qrl_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = qbit_pb2.GetTotalBalanceReq()
    ...
    def test_getOTS(self):
        with set_qrl_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = qbit_pb2.GetOTSReq(address=alice_xmss.address)
    ...
    def test_getBlock(self):
        with set_qrl_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = qbit_pb2.GetBlockReq(header_hash=b.headerhash)
    ...
    def test_getBlockByNumber(self):
        with set_qrl_dir('no_data'):
            ...
            Block.put_block_number_mapping(db_state,
                                           b.block_number,
                                           qbit_pb2.BlockNumberMapping(headerhash=b.headerhash),
                                           None)
            db_state.get_block_by_number = MagicMock(return_value=b)

            context = Mock(spec=ServicerContext)
            request = qbit_pb2.GetBlockByNumberReq(block_number=b.block_number)
