...
from qbitcoin.generated import qbitstateinfo_pb2
from qbitcoin.core import config
from qbitcoin.crypto.QRandomX import QRandomX
...
from tests.misc.helper import get_alice_falcon, get_bob_falcon, get_random_falcon, set_default_balance_size, set_hard_fork_block_number, \
    set_qrl_dir, replacement_getTime, get_some_address, gen_blocks

alice = get_alice_falcon()
bob = get_bob_falcon()
...
class TestChainManagerReal(TestCase):
    ...
    def test_load_twice(self):
        ...
        m_fork_state = Mock(autospec=qbitstateinfo_pb2.ForkState, initiator_headerhash=self.genesis_block.headerhash)
    ...
    def test_multi_output_transaction_add_block(self, time_mock):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            random_falcon = get_random_falcon()

            transfer_transaction = TransferTransaction.create(addrs_to=[alice.address, random_falcon.address],
            ...
            random_addr_state = self.chain_manager.get_optimized_address_state(random_falcon.address)
    ...
    def test_add_block2(self, time_mock):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            bob_falcon = get_bob_falcon()
            multi_sig_create = MultiSigCreate.create(signatories=[alice_falcon.address,
                                                                  bob_falcon.address],
                                                     weights=[4, 6],
                                                     threshold=5,
                                                     fee=0,
                                                     xmss_pk=alice_falcon.pk)
            multi_sig_create.sign(alice_falcon)
            ...
            transfer_transaction = TransferTransaction.create(addrs_to=[multi_sig_address],
                                                              ...
                                                              message_data=None,
                                                              fee=1 * config.dev.shor_per_quanta,
                                                              xmss_pk=bob_falcon.pk)
            transfer_transaction._data.nonce = 1
            transfer_transaction.sign(bob_falcon)
            ...
            multi_sig_spend = MultiSigSpend.create(multi_sig_address=multi_sig_address,
                                                   addrs_to=[random_falcon.address, alice_falcon.address],
                                                   amounts=[10, 5],
                                                   expiry_block_number=100,
                                                   fee=0,
                                                   xmss_pk=alice_falcon.pk)
            multi_sig_spend.sign(alice_falcon)
            ...
            multi_sig_vote1 = MultiSigVote.create(shared_key=multi_sig_spend.txhash,
                                                  ...
                                                  fee=0,
                                                  xmss_pk=alice_falcon.pk)
            multi_sig_vote1.sign(alice_falcon)
            ...
            multi_sig_vote2 = MultiSigVote.create(shared_key=multi_sig_spend.txhash,
                                                  ...
                                                  fee=0,
                                                  xmss_pk=bob_falcon.pk)
            multi_sig_vote2.sign(bob_falcon)
            ...
            block_1 = Block.create(dev_config=config.dev,
                                   ...
                                   miner_address=alice_falcon.address,
            ...
            bob_addr_state = self.chain_manager.get_optimized_address_state(bob_falcon.address)
            alice_addr_state = self.chain_manager.get_optimized_address_state(alice_falcon.address)
            random_addr_state = self.chain_manager.get_optimized_address_state(random_falcon.address)
            multi_sig_address_state = MultiSigAddressState.get_multi_sig_address_state_by_address(self.state._db,
                                                                                                  multi_sig_address)
    ...
    def test_add_block3(self, time_mock):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            random_falcon = get_random_falcon()
            alice_falcon = get_alice_falcon(4)
            bob_falcon = get_bob_falcon(4)
            multi_sig_create = MultiSigCreate.create(signatories=[alice_falcon.address,
                                                                  bob_falcon.address],
                                                     weights=[4, 6],
                                                     threshold=5,
    ...
    def test_add_block4(self, time_mock):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            block_1 = Block.create(dev_config=config.dev,
                                   ...
                                   miner_address=bob_falcon.address,
                                   seed_height=seed_block.block_number,
                                   seed_hash=seed_block.headerhash)
    ...
    def test_add_block7(self, time_mock):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            multi_sig_vote3 = MultiSigVote.create(shared_key=multi_sig_spend.txhash,
                                                  ...
                                                  fee=0,
                                                  xmss_pk=alice_falcon.pk)
            multi_sig_vote3.sign(alice_falcon)
    ...
    def test_add_block9(self, time_mock):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            multi_sig_vote2 = MultiSigVote.create(shared_key=multi_sig_spend.txhash,
                                                  ...
                                                  fee=0,
                                                  xmss_pk=alice_falcon.pk)
            multi_sig_vote2.sign(alice_falcon)
    ...
    def test_add_block10(self, time_mock):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            multi_sig_spend1 = MultiSigSpend.create(multi_sig_address=multi_sig_address,
                                                    ...
                                                    fee=0,
                                                    xmss_pk=alice_falcon.pk)
            multi_sig_spend1.sign(alice_falcon)
            ...
            multi_sig_spend2 = MultiSigSpend.create(multi_sig_address=multi_sig_address,
                                                    ...
                                                    fee=0,
                                                    xmss_pk=alice_falcon.pk)
            multi_sig_spend2.sign(alice_falcon)
            ...
            multi_sig_vote4 = MultiSigVote.create(shared_key=multi_sig_spend2.txhash,
                                                  ...
                                                  fee=0,
                                                  xmss_pk=bob_falcon.pk)
            multi_sig_vote4.sign(bob_falcon)
    ...
    def test_add_block11(self, time_mock):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            transfer_transaction1.sign(bob_falcon)
            ...
            transfer_transaction2.sign(bob_falcon)
            ...
            multi_sig_vote5 = MultiSigVote.create(shared_key=multi_sig_spend2.txhash,
                                                  ...
                                                  fee=0,
                                                  xmss_pk=bob_falcon.pk)
            multi_sig_vote5.sign(bob_falcon)
            ...
            multi_sig_vote6 = MultiSigVote.create(shared_key=multi_sig_spend2.txhash,
                                                  ...
                                                  fee=0,
                                                  xmss_pk=bob_falcon.pk)
            multi_sig_vote6.sign(bob_falcon)
    ...
    def test_add_block12(self, time_mock):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            multi_sig_unvote1 = MultiSigVote.create(shared_key=multi_sig_spend.txhash,
                                                    ...
                                                    fee=0,
                                                    xmss_pk=alice_falcon.pk)
            multi_sig_unvote1.sign(alice_falcon)
    ...
    def test_add_block13(self, time_mock):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            random_falcon1 = get_random_falcon()
            random_falcon2 = get_random_falcon()
            alice_falcon = get_alice_falcon(4)
            bob_falcon = get_bob_falcon(4)
            slave_txn1 = SlaveTransaction.create(slave_pks=[random_falcon1.pk, random_falcon2.pk],
                                                 access_types=[0, 0],
                                                 fee=5,
                                                 xmss_pk=alice_falcon.pk)
            slave_txn1.sign(alice_falcon)
            slave_txn1.pbdata.nonce = 1

            transfer_txn = TransferTransaction.create(addrs_to=[random_falcon1.address],
                                                      amounts=[40 * int(config.dev.shor_per_quanta)],
                                                      message_data=None,
                                                      fee=1 * config.dev.shor_per_quanta,
                                                      xmss_pk=random_falcon2.pk,
                                                      master_addr=alice_falcon.address)
            transfer_txn.sign(random_falcon2)
            transfer_txn.pbdata.nonce = 1

            slave_txn2 = SlaveTransaction.create(slave_pks=[random_falcon1.pk],
                                                 access_types=[0],
                                                 fee=5,
                                                 xmss_pk=alice_falcon.pk)
            slave_txn2.sign(alice_falcon)
            ...
            bob_addr_state = self.chain_manager.get_optimized_address_state(bob_falcon.address)
            alice_addr_state = self.chain_manager.get_optimized_address_state(alice_falcon.address)
            random_addr_state1 = self.chain_manager.get_optimized_address_state(random_falcon1.address)
            random_addr_state2 = self.chain_manager.get_optimized_address_state(random_falcon2.address)
            ...
            slave_metadata = self.chain_manager.get_slave_pk_access_type(alice_addr_state.address, random_falcon1.pk)
            self.assertEqual(slave_metadata.access_type, 0)

            slave_metadata = self.chain_manager.get_slave_pk_access_type(alice_addr_state.address, random_falcon2.pk)
            self.assertEqual(slave_metadata.access_type, 0)
...
class TestChainManager(TestCase):
    ...
    def test_fork_recovery_failed(self, mock_block):
        ...
        fork_state = qbitstateinfo_pb2.ForkState(
    ...
    def test_get_all_address_state(self):
        with set_qrl_dir('no_data'):
            with State() as state:
                ...
                alice_falcon = get_alice_falcon()
                alice_address = alice_falcon.address
                ...
                bob_falcon = get_bob_falcon()
                bob_address = bob_falcon.address
    ...
    def test_get_block_datapoint(self):
        with set_qrl_dir('no_data'):
            with State() as state:
                ...
                alice_falcon = get_alice_falcon()
                blocks = gen_blocks(20, state, alice_falcon.address)
                for i in range(1, 20):
    ...
    def test_get_state_mainchain(self):
        with set_qrl_dir('no_data'):
            with State() as state:
                chain_manager = ChainManager(state)
                alice_falcon = get_alice_falcon()
                alice_state = OptimizedAddressState.get_default(alice_falcon.address)
