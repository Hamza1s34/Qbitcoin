...
class TestTokenTransaction(TestCase):
    ...
    def setUp(self):
        self.initial_balances_valid = [qbit_pb2.AddressAmount(address=self.alice.address, amount=1000),
                                       qbit_pb2.AddressAmount(address=self.bob.address, amount=1000)]
    ...
    def test_create(self, m_logger):
        ...
        initial_balances.append(qbit_pb2.AddressAmount(address=self.alice.address,
                                                      amount=400000000))
        initial_balances.append(qbit_pb2.AddressAmount(address=self.bob.address,
                                                      amount=200000000))
    ...
    def test_validate_extended(self, m_validate_slave, m_logger):
        ...
        with patch('qrl.core.txs.TokenTransaction.TokenTransaction.initial_balances',
            ...
            m_address_balance.return_value = [qbit_pb2.AddressAmount(address=b'Invalid Address 1', amount=1000),
                                              qbit_pb2.AddressAmount(address=b'Invalid Address 2', amount=1000)]
            result = tx._validate_extended(state_container)
            self.assertFalse(result)
        ...
        with self.assertRaises(ValueError):
            initial_balances_0_0 = [qbit_pb2.AddressAmount(address=self.alice.address, amount=0),
                                    qbit_pb2.AddressAmount(address=self.bob.address, amount=0)]
    ...
    def test_affected_address(self, m_logger):
        ...
        result = set()
        self.initial_balances_valid.append(qbit_pb2.AddressAmount(address=slave.address, amount=1000))
        tx = TokenTransaction.create(symbol=b'QRL',
                                     name=b'Quantum Resistant Ledger',
                                     owner=self.alice.address,
                                     decimals=15,
