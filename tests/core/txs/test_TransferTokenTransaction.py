...
class TestTransferTokenTransaction(TestCase):
    ...
    def test_validate_extended(self, m_validate_slave, m_logger):
        ...
        tokens.data[(self.alice.address, tx.token_txhash)] = qbit_pb2.TokenBalance(balance=1000,
                                                                                  decimals=0,
                                                                                  delete=False)
        ...
        state_container.tokens.data[(self.alice.address, tx.token_txhash)] = qbit_pb2.TokenBalance(balance=1000,
                                                                                                  decimals=0,
                                                                                                  delete=False)
        ...
        tokens = Indexer(b'token', None)
        tokens.data[(self.alice.address, tx.token_txhash)] = qbit_pb2.TokenBalance(balance=99,
                                                                                  decimals=0,
                                                                                  delete=False)
        state_container.tokens = tokens
        result = tx._validate_extended(state_container)
