...
from qbitcoin.core.Message import Message
from qbitcoin.core.MessageRequest import MessageRequest
from qbitcoin.core.misc import logger
from qbitcoin.core.node import POW
...
class TestP2PFactory(TestCase):
    ...
    def test_request_full_message(self, m_reactor, m_logger):
        ...
        mrData = qbitlegacy_pb2.MRData(type=qbitlegacy_pb2.LegacyMessage.SL, hash=b'1234')
    ...
    def test_broadcast(self, m_reactor, m_logger):
        # broadcast msg_type is qbitlegacy_pb2.LegacyMessage
        self.factory.broadcast(qbitlegacy_pb2.LegacyMessage.TX, b'1234')
        self.channel_1.send.assert_called_once()
        self.channel_2.send.assert_called_once()
    ...
    def test_broadcast_does_not_broadcast_to_peers_known_to_have_the_mr(self, m_reactor, m_logger):
        ...
        self.factory.master_mr.requested_hash[b'1234'] = message_request
        self.factory.broadcast(qbitlegacy_pb2.LegacyMessage.SL, b'1234')
        self.channel_1.send.assert_called_once()
...
class TestP2PFactoryCompareAndSync(TestCase):
    def setUp(self):
        ...
        self.node_header_hash = qbit_pb2.NodeHeaderHash(
            block_number=1,
...
class TestP2PFactoryPeerFetchBlock(TestCase):
    def setUp(self):
        ...
        self.factory._target_node_header_hash = qbit_pb2.NodeHeaderHash(
            block_number=1,
