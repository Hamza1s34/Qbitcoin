...
from qbitcoin.crypto.misc import merkle_tx_hash
...
@patch('qbitcoin.core.misc.ntp.getTime', new=replacement_getTime)
...
class TestBlock(TestCase):
    ...
    def test_validate(self):
        ...
        with patch('qbitcoin.core.Block.Block.transactions', new_callable=PropertyMock) as m_transactions:
            m_transactions.return_value = []
            result = self.block.validate(m_chain_manager, OrderedDict())
            self.assertFalse(result)

        # There was a problem with the CoinBase TX
        with patch('qbitcoin.core.txs.CoinBase.CoinBase._validate_extended') as m_validate_extended:
...
@patch('qbitcoin.core.misc.ntp.getTime', new=replacement_getTime)
@patch('qbitcoin.core.txs.CoinBase.CoinBase.apply', name='mock CoinBase.apply()',
       return_value=True)
@patch('qbitcoin.core.txs.TransferTransaction.TransferTransaction.apply',
       name='mock TransferTransaction.apply()', return_value=True)
@patch('qbitcoin.core.txs.Transaction.Transaction.validate_slave',
       name='mock Transaction.validate_slave()', return_value=True)
@patch('qbitcoin.core.txs.TransferTransaction.TransferTransaction.validate', name='mock Transfer.validate()',
       return_value=True)
class TestBlockApplyStateChanges(TestCase):
    ...
    def test_put_block_number_mapping(self,
                                      ...
                                      m_TransferTransaction_apply_state_changes,
                                      m_CoinBase_apply_state_changes):
        bm = qbit_pb2.BlockNumberMapping()
    ...
    def test_last_block(self,
        ...
        with patch("qbitcoin.core.Block.Block.get_block_by_number") as mock_get_block_by_number:
